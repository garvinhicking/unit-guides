...
#
# $ docker run --rm -v $(pwd):/project -it phpunit:local --progress SomeDocsFolder"
#
...
#
# $ docker run --rm -v $(pwd):/project -it phpunit:local verbose migrate SomeDocsFolderWithSettings"
#
...
if [ "${SHELL_VERBOSITY}" -gt 0 ]; then
    ...
    echo "CDN VERSION:           ${AZUREEDGEURIVERSION}"
fi
...
if [ "${MY_UID}" -eq "0" ]; then
    ...
    if [ "$UID" -eq "0" ]; then
        ...
        addgroup phpunit --gid=$GID;
        adduser -h $(pwd) -D -G phpunit --uid=$UID phpunit;
        ...
        echo "export AZUREEDGEURIVERSION=${AZUREEDGEURIVERSION}"  >> /usr/local/bin/invocation.sh
        ...
        su - phpunit -c "/usr/local/bin/invocation.sh"
    fi
else
    if [ "${SHELL_VERBOSITY}" -gt 0 ]; then
